...
import delivery.interfaces.Filterable;
import delivery.interfaces.MyLinkedListOperator;
import delivery.interfaces.Reducer;
...
public class MyLinkedList<T> implements Iterable<T> {
    ...
    // so we can call orders.countElements(lambda) ?
    public <R> R countElements(MyLinkedListOperator<T, R> operator) {
        return operator.operate(this);
    }

    public <R> R reduce(R initial, Reducer<T, R> reducer) {
    R result = initial;
    for (T element : this) {
    ...
    // return a new list using the Func Interface?
    public MyLinkedList<T> filter(Filterable<T> impl){
        MyLinkedList<T> result = new MyLinkedList<>();
        for(T element: this){
            if(impl.test(element)){
                result.add(element);
            }
        }
        return result;
